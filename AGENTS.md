...
- VM side: systemd user service in `users/m/home-manager/desktop.nix` — `uniclip` (client connecting to `127.0.0.1:53701`)
...
| `/users/m/home-manager/` | Composable home-manager modules (core, shell, git, dev, desktop, ai-agents) |
...
| `/users/m/opencode/` | OpenCode AI assistant configuration and plugin aggregation |
| `/patches/` | Source patches (uniclip bind+env password patch) |
| `/docs/` | Documentation and helper scripts (`vm.sh`, `macbook.sh`, `wsl.sh`, `rpi.sh`) |
...
| File | Purpose |
|------|---------|
| `home-manager.nix` | Orchestrator — imports composable sub-modules from `home-manager/` |
| `home-manager/core.nix` | Base packages, env vars, dotfiles, activation hooks (every machine) |
| `home-manager/shell.nix` | Zsh, bash, aliases, tmux, direnv, prompt tools (every machine) |
| `home-manager/git.nix` | Git, gh, GPG, gpg-agent, rbw (every machine) |
| `home-manager/dev.nix` | Language runtimes, editors: Doom Emacs, LazyVim, VSCode (dev machines) |
| `home-manager/desktop.nix` | Niri, Mango, Noctalia, browsers, screenshots, services (GUI machines) |
| `home-manager/ai-agents.nix` | LLM agents, OpenCode, Claude Code, wrapper scripts (dev machines) |
...
| `ghostty.linux.cfg` | Ghostty terminal configuration for Linux |
| `ghostty.darwin.cfg` | Ghostty terminal configuration for macOS |
| `bashrc`, `inputrc`, `gdbinit` | Shell and tool configurations |

## Build & Deployment Scripts

Helper scripts live in `docs/` and are used for system management:

| Script | Description |
|--------|-------------|
| `docs/vm.sh` | VM lifecycle management (create, start, stop, IP detection, ssh, copy, switch) |
| `docs/macbook.sh` | macOS setup and nix-darwin rebuild automation |
| `docs/wsl.sh` | WSL provisioning and tarball building |
| `docs/rpi.sh` | Raspberry Pi setup (provisioning) |

**Applying configuration changes:**
```bash
# NixOS VM (from inside the VM)
sudo nixos-rebuild switch --flake '/nixos-config#vm-aarch64'

# macOS (from the host)
nix build '.#darwinConfigurations.macbook-pro-m1.system' && sudo ./result/sw/bin/darwin-rebuild switch --flake '.#macbook-pro-m1'
...
# Rebuild manually
cd ~/.config/nix && nix build '.#darwinConfigurations.macbook-pro-m1.system' && sudo ./result/sw/bin/darwin-rebuild switch --flake '.#macbook-pro-m1'
```
...
1. **Determine scope:**
   ...
   - User-only (choose the right sub-module):
     - Core CLI tools: `users/m/home-manager/core.nix`
     - Dev tools / editors: `users/m/home-manager/dev.nix`
     - Desktop/GUI apps: `users/m/home-manager/desktop.nix`
     - AI agents: `users/m/home-manager/ai-agents.nix`
...
4. **Test:** `nix build .#nixosConfigurations.<name>.config.system.build.toplevel`

5. **Remember:** New files must be `git add`ed before building — Nix flakes only see tracked files in dirty git trees.

### Modifying Shell Configuration

- **Aliases:** Edit `shellAliases` in `users/m/home-manager/shell.nix`
- **Zsh settings:** Edit `programs.zsh` in `users/m/home-manager/shell.nix`
- **Zsh init:** Edit `programs.zsh.initContent` (platform-conditional blocks for brew shellenv, rbw wrappers, etc.)
- **Bash settings:** Edit `programs.bash` in `users/m/home-manager/shell.nix` or `users/m/bashrc`
...
- See `docs/secrets.md` for the full secrets workflow
...
11. **New files must be git-added** - Nix flakes only see tracked files in dirty git trees
12. **nixpkgs 25.11 changes** - `darwin.apple_sdk_11_0` throws on access (use default `apple-sdk-14.4`); `du-dust` renamed to `dust`; `activitywatch` is Linux-only (use homebrew cask on Darwin)
13. **Modular home-manager** - User config is split into composable modules under `users/m/home-manager/`. For headless machines, create a slimmer orchestrator importing only the needed modules (e.g. core + shell + git)
